...
import chap22_lambda.reference.MoneyConverter;
...
public class _08_ReferenceMethod {
	public static void main(String[] args) {
		...
		// 1. 기존에 배웠던 람다식으로 구성
//		convertMoney((a, b) -> MoneyConverter.staticConvert(a, b));
...
//		convertMoney(MoneyConverter::staticConvert);
		
		// 4. 일반(인스턴스) 메소드를 참조하려면 참조할 메소드를 사용할 객체를 먼저 생성해야한다.
		MoneyConverter moneyConverter = new MoneyConverter();
		// 참조방식은 클래스타입의 변수명::참조할 메소드명
//		convertMoney((a, b) -> moneyConverter.instanceConvert(a, b)); 
		convertMoney(moneyConverter::instanceConvert);
		
		// 5. 추상메소드와 형태가 다른 메서드는 참조해서 사용할 수 없다.
//		convertMoney(moneyConverter::otherConvert);
		
		// 6. 추상메소드와 형태가 다른 메소드는 람다식으로 구성해서 사용한다.
		convertMoney((a, b) -> {
			moneyConverter.otherConvert(a, b, "aaaa");
			return 1000;
		});
	}
	
	public static void convertMoney(Converter converter) {
		...
		int result = converter.convert(money, str.charAt(0));
