...
public class _19_GroupingBy {
	public static void main(String[] args) {
		...
		creditCardList.add(new CreditCard("현대카드", "더블랙", 30000));
		creditCardList.add(new CreditCard("현대카드", "더블랙", 50000));
		...
		// Collectors.groupingBy(키값으로 지정할 람다식, Collectors.averagingDouble(람다식));
		...
		Map<String, Long> companyCardCountingMap = 
				creditCardList.stream()
							  .collect(
									  Collectors.groupingBy(card->card.getCompany(),
											  Collectors.counting())
									  );
		
		System.out.println(companyCardCountingMap);
	}
}
