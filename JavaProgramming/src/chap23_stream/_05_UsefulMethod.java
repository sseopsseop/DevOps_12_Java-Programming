...
public class _05_UsefulMethod {
	public static void main(String[] args) {
		...
		intList.stream()
				// intList에 담긴 요소드르이 제곱들로 구성된 새로운 Stream 리턴
			   .map(num -> num * num)
			   .forEach(num -> System.out.println(num));

		System.out.println("---------------------------");
		// intList에 담긴 값 중 짝수는 제곱으로 변경하고 홀수는 값 그대로
		// forEach로 출력
		intList.stream()
...
//		   .forEach(num -> System.out.println(num));
		...
		System.out.println("모두 곱한 값 : " + intList.stream()
												   .reduce(1, (mulVal, num) -> mulVal * num)
												   .intValue());
		// intList에서 짝수는 /2 홀수는 /3한 값들을 모두 더한 값을 구하세요.(map, reduce 사용)
		int res = intList.stream()
			   .map(num ->{
				   if(num % 2 == 0) num /= 2;
				   else num /= 3;
			   ...
			   .reduce(0, (sumVal, num) -> sumVal + num).intValue();
